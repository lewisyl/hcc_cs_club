...
public function show() {
   ...
   foreach ($db->query("SELECT * FROM events") as $row){
       ...
       if (array_key_exists($row['date'], $dates)){
           array_push($dates[$row['date']], $event);
       }
       else{
...
if($appointment != NULL){
    $c = '<li id="li-'.$this->currentDate.'" class="'.($cellNumber%7==1?' 
 start ':($cellNumber%7==0?' end ':' ')).
        ($cellContent==null?'mask':'').' busy"><span class="eventToday">'.$cellContent.'</span>';
        $c.= "<div class='calendarEventTitle'>"; 
        for($w=0;$w<count($appointment);$w++){
            $c.= //"<li> - ".
            $appointment[$w]['title'];
            $appointment[$w]['detail'];
            //."</li>";
        }
        $c.= "</div>";
    ...
    $c = '<li id="li-'.$this->currentDate.'" class="'.($cellNumber%7==1?' 
 start ':($cellNumber%7==0?' end ':' ')).
        ($cellContent==null?'mask':'').' free"><span>'.$cellContent.'</span></li>';
}    
...
private function _createNavi(){
    ...
    return
        '<div class="calendarHeader">'.
            '<button class="prev"><a href="'.$this->naviHref.'? 
   month='.sprintf('%02d',$preMonth).'&year='.$preYear.'">&#8678</a></button>'.
                '<span class="title">'.date('Y M',strtotime($this->currentYear.'-'.$this->currentMonth.'-1')).'</span>'.
            '<button class="next"><a href="'.$this->naviHref.'? 
           month='.sprintf("%02d", 
         $nextMonth).'&year='.$nextYear.'">&#8680</a></button>'.
        '</div>';
}
